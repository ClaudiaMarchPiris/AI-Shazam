...
n_epoch       = 1000
n_samples     = 625                              # change to 1000 for entire dataset
cv_split      = 1                             
...
def cnn(melspectrogram, weights, phase_train):
    ...
    dropout_1 = tf.nn.dropout(mpool_1, pdropout)
    ...
    dropout_2 = tf.nn.dropout(mpool_2, pdropout)
    ...
    dropout_3 = tf.nn.dropout(mpool_3, pdropout)
    ...
    dropout_4 = tf.nn.dropout(mpool_4, pdropout)
    ...
    dropout_5 = tf.nn.dropout(mpool_5, pdropout)
...
if __name__ == '__main__':
    ...
    weights = {
        ...
        'woutput':init_weights([256, 30]),
        'boutput':init_biases([30])}

    X = tf.placeholder("float", [None, 96, 1366, 1])
    y = tf.placeholder("float", [None, 30])
    ...
    with tf.Session() as sess:
        ...
        saver = tf.train.import_meta_graph('../data/Model/SongModel.ckpt-10.meta')
        ...
        for i in range(n_epoch):
            ...
            for start, end in training_batch:
                ...
                if (i % 10 == 0):
                    _, cost_traning, predictions = sess.run([train_op, cost, predict_op], feed_dict=train_input_dict)
                    #print("Traning Cost :",cost_encountered)
                    #print(training_batch[0])
                    auc = sm.roc_auc_score(y_train[start:end], predictions, average='samples')
                    #print('Train : ','AUC : ', auc,"loss:",cost_traning)
                    total_loss += cost_traning
                    total_accuracy += auc
                else:
            ...
            test_input_dict = {X: X_test[test_indices],
                               ...
                               phase_train:True}
            predictions, cost_encountered = sess.run([predict_op ,cost], feed_dict=test_input_dict)
            print('Epoch : ', i,  'AUC : ', sm.roc_auc_score(y_test[test_indices], predictions, average='samples'),"loss:",cost_encountered)
            print("")
            print("")
            print("")'''
            if (i%10 == 0):
                print("epoch"+str(i))
                print("accuracy =",total_accuracy/(train_size/batch_size*1.0))
                print("loss =",total_loss/(train_size/batch_size*1.0))
                print(total_loss,total_accuracy)
                saver.save(sess, "../data/Model/SongModel.ckpt",global_step = 10)
